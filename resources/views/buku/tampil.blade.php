...
@section('judul')
    <h1 style="color: #10487a;">Daftar Buku</h1>
@endsection

@section('content')
    @if (Auth::user()->isAdmin == 1)
        <a href="/buku/create" class="btn mb-3" style="background-color: #10487a; color: #fff; border: none;"><i class="fa-solid fa-plus"></i> Tambah Buku</a>
    @endif
    ...
    <form class="navbar-search mb-3" action="/buku" method="GET">
        <div class="input-group">
            <input type="search" name="search" class="form-control bg-light border-1 small" placeholder="Cari Judul Buku"
                aria-label="Search" aria-describedby="basic-addon2" style="border-color: #10487a;">
            <div class="input-group-append">
                <button class="btn" style="background-color: #10487a; color: #fff; border: none;" type="submit">
    ...
    <div class="card container-fluid mb-3">
                        ...
                        <div class="card-body d-flex flex-column justify-content-between">
                            <div class="detai-buku">
                                <h5 class="card-title"><a
                                        href="/buku/{{ $item->id }}"style="text-decoration: none; font-size:1rem;font-weight:bold; color: #10487a;">
                                        {{ $item->judul }}</a></h5>
                                <p class = "cart-text m-0">Kode Buku : <a style="color: #10487a;">{{ $item->kode_buku }}</a></p>
                                <p class="card-text m-0">Pengarang : <a style="text-decoration: none; color: #10487a;">{{ $item->pengarang }}</a></p>
                                <p class="card-text m-0">Kategori : <a style="color: #10487a;">
                                    @foreach ($item->kategori_buku as $kategori )
                                    {{ $kategori->nama, }}
                                    @endforeach</a></p>
                                <p class="card-text m-0">Status : <a style="color: #10487a;">{{$item->status  }}</a></p>
                            </div>
                            @if (Auth::user()->isAdmin == 1)
        ...
        <div class="d-flex justify-content-between mx-2 my-2">
            <p class="my-2" style="color: #10487a;">Menampilkan {{ $buku->currentPage() }} dari {{ $buku->lastPage() }} Halaman</p>
